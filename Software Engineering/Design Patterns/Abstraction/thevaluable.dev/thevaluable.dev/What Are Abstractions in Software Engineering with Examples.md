...
* To actually wash the clothes, it is not necessary to know how the washing machine works internally
  * In other words, it is not necessary to know about the drums, water valves, thermostat, etc. and how they work together in order to wash these clothes
 
## Control Abstraction
* Function names should simplify and hide internal details
* The function name + the function inputs + the function outputs (i.e. the function signature) should provide all the necessary details
...
* Implementing a concrete `YamlParser` class that implements a `Parser` "interface construct" does not necessarily hide details, since all `Parser` methods need to be implemented
  * `YamlParser` is also not generalized, for the same reason (there is a specific implementation for `YAML` parsing)
* Instead, using the "indirect" "interface construct" adds flexibility - the `YamlParser` can be swapped with a `JsonParser` if they both implement the `Parser` interface
* Abstractions may need to be repaired, so developers must have some knowledge about what is behind the abstraction, i.e. the underlying complexity that the abstraction is trying to hide, in order to understand how to fix the abstraction when it "leaks"
